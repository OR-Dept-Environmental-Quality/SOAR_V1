...
# # additional sites to the query using this function
# ext_deq_dat <- function(i_dat_request, allsite,
#                         poll_name = 'none', 
#                         from_date = '01/01/01', 
#                         to_date = '01/01/01', 
#                         by_date = 'hour'){
#   
#   # check input & control here
#   if(allsite[1] == 'none'){stop('ERROR: set site with 3 letter code')}
#   if(poll_name == 'none'){stop('ERROR: set pollutant type: pm25, o3, nox, sensor, etc')}
#   if(to_date == '01/01/01'){stop('ERROR: set time range')}
#   
#   meta <- as.data.frame(matrix(ncol = 12, nrow = length(allsite)))
#   names(meta) <- c('site', 'epa_id', 'lat', 'long', 'poll_name', 'poll_envista', 'poll_aqs', 'poll_alias', 
#                    'from_date', 'to_date', 'by_date',
#                    'method_env')
#   
#   for(ind in 1:length(allsite)){
...
#     
#     try({
#       meta$site[ind]         <- tolower(i_site)
#       meta$epa_id[ind]       <- cross_tables$aqm_sites$epa[cross_tables$aqm_sites$site == i_site]
#       meta$lat[ind]          <- cross_tables$aqm_sites$latitude[cross_tables$aqm_sites$site == i_site]
#       meta$long[ind]         <- cross_tables$aqm_sites$longitude[cross_tables$aqm_sites$site == i_site]
#       meta$poll_name[ind]    <- tolower(poll_name)
#       meta$poll_envista[ind] <- tolower(cross_tables$analyteXcode$EnvistaName[cross_tables$analyteXcode$name == poll_name])
#       meta$poll_aqs[ind]     <- tolower(cross_tables$analyteXcode$AqsName[cross_tables$analyteXcode$name == poll_name])
#       meta$poll_alias[ind]   <- tolower(cross_tables$analyteXcode$Alias[cross_tables$analyteXcode$name == poll_name])
#       meta$method_env[ind]   <- cross_tables$analyteXcode$method_code_filter[cross_tables$analyteXcode$name == poll_name]
#       meta$from_date[ind]    <- from_date
#       meta$to_date[ind]      <- to_date
#       meta$by_date[ind]      <- by_date
#     }, silent = TRUE)
#   }
#   
#   i_dat_request$meta <- bind_rows(i_dat_request$meta, meta)
#   
#   return(i_dat_request)}