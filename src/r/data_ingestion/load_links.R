...
loadXlink <- function(root_to_crosstab){
  ...
  aqm_sites <- read.csv(paste0(root_to_crosstab, "siteXepaid_crosstable.csv"))
  ...
  analyteXcode <- read.csv(paste0(root_to_crosstab, "analyteXcode.csv"))
  
  qualifierXcode <- read.csv(paste0(root_to_crosstab, "/envista_api_qualifier_code.csv"))
